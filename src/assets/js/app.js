...
const app = {}
//该方法用于存储看板
app.setState = function (st1, str) {
  if (typeof str !== 'string') throw 'str is not a string!'
  localStorage.setItem(st1, str)
}
app.getState = function (st1) {
  return JSON.parse(localStorage.getItem(st1));
}
//该方法用于存储用户信息
      ...
      success: function (e) {
        ...
        if (result.canpass === 'null') {
          ...
          if (result.canpass === 'False') {
            mui.alert('后端报错:' + result.errormessage)
            failFunction()
            return;
          }
          global.context.sendRequestTimes = 0
...
app.gettwofixed2 = function (num) {
  num = num / 10000
  num = num.toFixed(2)
  return num;
}
app.gettwofixed4 = function (num) {
  num = Number(num).toFixed(4)
  return num;
}
//连接服务器失败
app.failToServer = function (xhr, type, errorThrown) {
  if (xhr.status === 0) {
    mui.alert('网络异常(错误码' + xhr.status + ")", '前台提示');
    return
  }
  if (xhr.status >= 500) {
    mui.alert('服务器异常(错误码' + xhr.status + ")", '前台提示');
    return
  }
