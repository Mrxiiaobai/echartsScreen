		...
		<div class="allcharts" v-bind:style="{height:win_height+'vh'}">
			...
			<div class="selectPart2" @click="nofunction()" style="left:30%!important;width:70%!important;">
				<div style="display: inline-block;" class="block" >
					<el-date-picker @change="select()" v-model="value7" type="daterange" align="right" unlink-panels range-separator="至"
			...
			<div id="KbinspprwcChart" v-show="showCharts"></div>
			<div id="KbinspprwcTable" v-show="showTable">
				<el-table :data="tableData" :span-method="objectSpanMethod" border style="width: 100%; margin-top: 20px;color:black;margin-bottom: 50px;">
...
<script>
	...
	export default {
		...
		methods: {
			...
			resizeHeight() {
				let _this = this
				document.getElementById("KbinspprwcChart").style.height = _this.chartHigh;
				document.getElementById("KbinspprwcChart").style.width = _this.chartWidth;
			...
			select() {
				...
				var data = {
					query: "t1_date between '" + _this.value7[0].getFullYear() + "-" + m1 + "-" + _this.value7[0].getDate() +
						"' and '" + _this.value7[1].getFullYear() + "-" + m2 + "-" + _this.value7[1].getDate() + "' and t1_sec ilike '" +
						_this.nowEm + "'"
				};
				mui.ajax({
					type: "post",
					url: global.context.webUrl + '/Kbinspprcget',
					...
					success: function(e) {
						...
						_this.series.submitDay = []
						_this.series.planDay = []
						...
						for (let i = 0; i < result.table1.length; i++) {
							...
							_this.series.submitInspectPlanData.push(ct1_plan_count)
							_this.series.submitInspectActualPerformceData.push(ct1_count_qty)
							_this.series.submitDay.push(ct1_day_qty)
							// _this.series.planDay.push(ct1_plan_qty)
							_this.top = ct1_plan_qty

							_this.series.submitInspectRate.push(ct1_chk_rat)
							let amount = _index,
								value = ct1_plan_qty;
							...
							_this.tableData[4][amount - 1] = ct1_chk_rat
							_index++
						}
						//上月在制品
			...
			generateChart() {
				document.getElementById("KbinspprwcChart").style.height = this.chartHigh;
				document.getElementById("KbinspprwcChart").style.width = this.chartWidth;
				let _this = this;
				// if (_this.chartDOM != {}) {
				//   _this.chartDOM.dispose();
				// }
				// document.getElementById("chart").style.height = document.documentElement.clientHeight - 50 + "px";
				document.getElementById("KbinspprwcChart").style.width = '100%';
				_this.$nextTick(() => {
					_this.chartDOM = _this.$echarts.init(document.getElementById("KbinspprwcChart"), 'theme');
			...
			getOffice() {
				...
				mui.ajax({
					type: "post",
					url: global.context.webUrl + '/Kbinspprcsecget',
					beforeSend: function(xhr) {
						xhr.setRequestHeader("accept", "application/json");
						xhr.setRequestHeader("Content-Type", "application/json");
					},
		...
		mounted() {
			...
			// this.chartHigh = this.kbpgrmd_win_height-5 + '%'
